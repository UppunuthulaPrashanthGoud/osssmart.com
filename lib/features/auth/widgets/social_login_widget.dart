    ...
    if (isRoute) {
      if (token != null) {
        ...
        if (Provider.of<SplashController>(context, listen: false).configModel!.emailVerification!) {
          Provider.of<AuthController>(context, listen: false).sendOtpToEmail(
            socialLogin.email.toString(),
            temporaryToken,
          ).then((value) async {
            if (value.response?.statusCode == 200) {
              ...
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (_) => VerificationScreen(temporaryToken, '', socialLogin.email.toString())),
                    (route) => false,
              );
            }
          });
        ...
        Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(builder: (_) => MobileVerificationScreen(temporaryToken)),
              (route) => false,
        );
      } else {
              ...
              Provider.of<SplashController>(context, listen: false).configModel!.socialLogin![1].status!
                ...
                onTap: () async {
                  ...
                  if (Provider.of<FacebookLoginController>(Get.context!, listen: false).userData != null) {
                    var accessToken = Provider.of<FacebookLoginController>(Get.context!, listen: false).result.accessToken;
                    id = accessToken?.toJson()['userId'];
                ...
                child: Padding(
                  ...
                  : const SizedBox(),
              if (Provider.of<SplashController>(context, listen: false).configModel!.socialLogin!.length > 2 &&
                  Provider.of<SplashController>(context, listen: false).configModel!.socialLogin![2].status! &&
                  Platform.isIOS)
                Padding(
                    ...
                    child: SizedBox(
                      ...
                      child: ClipRRect(
                        ...
                        child: SignInWithAppleButton(
                          style: Provider.of<ThemeController>(context, listen: false).darkTheme
